...
def add_ons_func():
    ...
    addon_input = input()  
    if addon_input.upper() == 'SKIP': 
        main_menu_func()
    else:
        addon_id = int(addon_input) 
...
def drinks_func():
    ...
    drink_input = input() 
    if drink_input.upper() == 'SKIP': 
        main_menu_func()
    else:
        drink_id = int(drink_input)  
...
if coin_type == "YES": 
    ...
    print('\*¸ „„.•~¹°”ˆ˜¨♡ Sorry, but we only accept DIRHAMS. TRY AGAIN!:3 ♡¨˜ˆ”°¹~•.„¸*')